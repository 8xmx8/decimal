...
var testTable = []*testEnt{
	...
	// go Issue 29491.
	{498484681984085570, "498484681984085568", "", ""},
...
func init() {
	...
	// -float64(MaxInt64) round-trips through int64 exactly; NewFromFloat keeps
	// the full integer digits, not the same as strconv.FormatFloat's shortest.
	negMaxBits := math.Float64bits(-float64(math.MaxInt64))
	for _, x := range testTable {
		if math.Float64bits(x.float) == negMaxBits {
			x.short = "-9223372036854775808"
		}
	}
